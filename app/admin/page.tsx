...
function defaultDrawLabel(): string {
  const now = new Date();
  return `${now.toLocaleDateString(undefined, { weekday: "long" })} ${now.toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" })} Vegas Gold Call`;
}
...
export default function AdminPage() {
  ...
  const [drawLabel, setDrawLabel] = useState(defaultDrawLabel());
  ...
  async function handleRunRaffle() {
    ...
    try {
      const res = await fetch("/api/admin/raffle", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ password, drawLabel }),
      });
      const data = await res.json();
      if (!res.ok) {
      ...
      <div className="mx-auto max-w-2xl space-y-6">
          ...
          {stats && (
            <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3">
              <div className="rounded-xl border border-slate-800 bg-slate-950 p-4 text-center">
                <p className="text-3xl font-bold text-yellow-400">{stats.totalVisitors}</p>
                <p className="mt-1 text-xs text-slate-400">Total registered</p>
              </div>
              <div className="rounded-xl border border-slate-800 bg-slate-950 p-4 text-center">
                <p className="text-3xl font-bold text-yellow-400">{stats.fullyCollected}</p>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-md">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Picks a random winner weighted by tokens held (a visitor with more tokens is
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-md">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Download all visitor data as CSV for post-show contact and reconciliation.
          </p>
          <button
